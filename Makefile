...
test:
	$(MAKE) create-db
	$(MAKE) migrate || ($(MAKE) destroy-db && false)
	$(MAKE) validate || ($(MAKE) destroy-db && false)
	$(MAKE) destroy-db
...
create-db:
	...
	@i=1; while ! $(MAKE) --quiet init-db 2>/dev/null; do if [ $$i = 10 ]; then exit 1; fi; let i++; sleep 1; echo -n .; done; echo
